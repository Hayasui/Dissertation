...
def append_game_to_results(game_data, csv_path):
    ...
    if os.path.exists(csv_path) and os.path.getsize(csv_path) > 0:
        try:
            ...
            # Check if this app_id already exists - FIXED to avoid array ambiguity
            app_id_str = str(game_data['app_id'])
            if 'app_id' in existing_data.columns and (existing_data['app_id'].astype(str) == app_id_str).any():
                print(f"App ID {app_id_str} already exists in CSV. Updating existing entry.")
                # Remove existing entry - FIXED to avoid array ambiguity
...
def merge_tags(user_tags, genres):
    ...
    # Clean lists by removing None, empty strings, etc. - FIXED to avoid array ambiguity
    clean_user_tags = []
...
def save_data_for_r(data, base_filename):
    ...
    try:
        ...
        # Create R import script with improved list handling - FIXED REGEX ESCAPING
